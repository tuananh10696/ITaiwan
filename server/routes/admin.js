...
router.get('/stats', async (req, res) => {
  try {
    ...
    const [[{ vocabCount }]] = await pool.query('SELECT COUNT(*) as vocabCount FROM vocabulary');
    const [[{ examCount }]] = await pool.query('SELECT COUNT(*) as examCount FROM exam_questions');
    const [[{ dialogueCount }]] = await pool.query('SELECT COUNT(*) as dialogueCount FROM dialogues');
    const [[{ blogCount }]] = await pool.query('SELECT COUNT(*) as blogCount FROM blog_posts');
...
router.get('/vocabulary', async (req, res) => {
  try {
    const { search, level, lesson } = req.query;
    const page = toPage(req.query.page);
    ...
    const where = [];
    if (search) {
      where.push('(hanzi LIKE ? OR simplified LIKE ? OR pinyin LIKE ? OR meaning LIKE ?)');
      const s = `%${search}%`;
      params.push(s, s, s, s);
    }
    if (level) { where.push('level = ?'); params.push(level); }
    if (lesson) { where.push('lesson = ?'); params.push(lesson); }
    if (where.length) sql += ' WHERE ' + where.join(' AND ');

    // Support custom sort
    const { sortBy = 'id', sortDir = 'ASC' } = req.query;
    const allowedSort = ['id', 'hanzi', 'pinyin', 'meaning', 'level', 'lesson', 'sort_order'];
    const safeSortBy = allowedSort.includes(sortBy) ? sortBy : 'id';
    const safeSortDir = sortDir.toUpperCase() === 'DESC' ? 'DESC' : 'ASC';
    sql += ` ORDER BY ${safeSortBy} ${safeSortDir}, id ASC`;
    const offset = (page - 1) * limit;
    sql += ' LIMIT ? OFFSET ?';
    params.push(limit, offset);
    const [rows] = await pool.query(sql, params);
    ...
    // Count
    let countSql = 'SELECT COUNT(*) as total FROM vocabulary';
    const cp = [];
    const cw = [];
    if (search) { cw.push('(hanzi LIKE ? OR simplified LIKE ? OR pinyin LIKE ? OR meaning LIKE ?)'); const s = `%${search}%`; cp.push(s,s,s,s); }
    if (level) { cw.push('level = ?'); cp.push(level); }
    if (lesson) { cw.push('lesson = ?'); cp.push(lesson); }
    if (cw.length) countSql += ' WHERE ' + cw.join(' AND ');
    const [[{ total }]] = await pool.query(countSql, cp);

    res.json({ vocabulary: rows, total, page, limit });
  } catch (err) {
    console.error('Admin vocab error:', err);
...
router.post('/vocabulary', async (req, res) => {
  try {
    const { hanzi, simplified, pinyin, meaning, level, lesson, category, example_hanzi, example_meaning } = req.body;
    if (!hanzi || !pinyin || !meaning || !level || !lesson) {
      return res.status(400).json({ error: 'Vui lòng nhập đầy đủ: hanzi, pinyin, meaning, level, lesson.' });
    }
    const [result] = await pool.query(
      'INSERT INTO vocabulary (hanzi, simplified, pinyin, meaning, level, lesson, category, example_hanzi, example_meaning) VALUES (?,?,?,?,?,?,?,?,?)',
      [hanzi, simplified || hanzi, pinyin, meaning, level, lesson, category || '', example_hanzi || '', example_meaning || '']
    );
    res.status(201).json({ message: 'Thêm từ vựng thành công.', id: result.insertId });
  } catch (err) {
...
router.put('/vocabulary/:id', async (req, res) => {
  try {
    const { hanzi, simplified, pinyin, meaning, level, lesson, category, example_hanzi, example_meaning, sort_order } = req.body;
    await pool.query(
      'UPDATE vocabulary SET hanzi=?, simplified=?, pinyin=?, meaning=?, level=?, lesson=?, category=?, example_hanzi=?, example_meaning=?, sort_order=? WHERE id=?',
      [hanzi, simplified || hanzi, pinyin, meaning, level, lesson, category || '', example_hanzi || '', example_meaning || '', sort_order || 0, req.params.id]
    );
    res.json({ message: 'Cập nhật từ vựng thành công.' });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi cập nhật từ vựng.' });
  }
});
...
router.get('/exam-questions', async (req, res) => {
  try {
    ...
    if (level) { where.push('level = ?'); params.push(level); }
    if (where.length) sql += ' WHERE ' + where.join(' AND ');
    sql += ' ORDER BY type, level, id';
    const offset = (page - 1) * limit;
    sql += ' LIMIT ? OFFSET ?';
    params.push(limit, offset);
    const [rows] = await pool.query(sql, params);

    let countSql = 'SELECT COUNT(*) as total FROM exam_questions';
    const cp = []; const cw = [];
    if (type) { cw.push('type = ?'); cp.push(type); }
    if (level) { cw.push('level = ?'); cp.push(level); }
    if (cw.length) countSql += ' WHERE ' + cw.join(' AND ');
    const [[{ total }]] = await pool.query(countSql, cp);

    res.json({ questions: rows, total, page, limit });
  } catch (err) {
...
router.post('/exam-questions', async (req, res) => {
  try {
    const { type, level, passage, audio_desc, question, question_meaning, option_a, option_b, option_c, option_d, option_a_meaning, option_b_meaning, option_c_meaning, option_d_meaning, correct_option } = req.body;
    if (!type || !level || !question || !option_a || !option_b || !option_c || !option_d || correct_option === undefined) {
      return res.status(400).json({ error: 'Vui lòng nhập đầy đủ thông tin câu hỏi.' });
    }
    const [result] = await pool.query(
      ...
      [type, level, passage || null, audio_desc || null, question, question_meaning || '', option_a, option_b, option_c, option_d, option_a_meaning || '', option_b_meaning || '', option_c_meaning || '', option_d_meaning || '', parseInt(correct_option)]
    );
    res.status(201).json({ message: 'Thêm câu hỏi thành công.', id: result.insertId });
  } catch (err) {
    console.error('Add exam question error:', err);
...
router.put('/exam-questions/:id', async (req, res) => {
  try {
    const { type, level, passage, audio_desc, question, question_meaning, option_a, option_b, option_c, option_d, option_a_meaning, option_b_meaning, option_c_meaning, option_d_meaning, correct_option } = req.body;
    await pool.query(
      `UPDATE exam_questions SET type=?, level=?, passage=?, audio_desc=?, question=?, question_meaning=?, option_a=?, option_b=?, option_c=?, option_d=?, option_a_meaning=?, option_b_meaning=?, option_c_meaning=?, option_d_meaning=?, correct_option=? WHERE id=?`,
      [type, level, passage || null, audio_desc || null, question, question_meaning || '', option_a, option_b, option_c, option_d, option_a_meaning || '', option_b_meaning || '', option_c_meaning || '', option_d_meaning || '', parseInt(correct_option), req.params.id]
    );
    res.json({ message: 'Cập nhật câu hỏi thành công.' });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi cập nhật câu hỏi.' });
  }
});

router.delete('/exam-questions/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM exam_questions WHERE id = ?', [req.params.id]);
    res.json({ message: 'Xóa câu hỏi thành công.' });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi xóa câu hỏi.' });
  }
});
...
router.get('/dialogues', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT d.*, (SELECT COUNT(*) FROM dialogue_lines dl WHERE dl.dialogue_id = d.id) AS line_count FROM dialogues d ORDER BY sort_order, id');
...
router.get('/dialogues/:id', async (req, res) => {
  try {
    const [dRows] = await pool.query('SELECT * FROM dialogues WHERE id = ?', [req.params.id]);
    if (!dRows.length) return res.status(404).json({ error: 'Không tìm thấy hội thoại.' });
    const [lines] = await pool.query('SELECT * FROM dialogue_lines WHERE dialogue_id = ? ORDER BY line_order', [req.params.id]);
    res.json({ dialogue: dRows[0], lines });
  } catch (err) {
...
router.post('/dialogues', async (req, res) => {
  try {
    const { title, title_cn, level, sort_order } = req.body;
    if (!title || !title_cn || !level) return res.status(400).json({ error: 'Vui lòng nhập đầy đủ tiêu đề, tiêu đề Trung và cấp độ.' });
    const [result] = await pool.query(
      'INSERT INTO dialogues (title, title_cn, level, sort_order) VALUES (?,?,?,?)',
      [title, title_cn, level, sort_order || 0]
    );
    res.status(201).json({ message: 'Thêm hội thoại thành công.', id: result.insertId });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi thêm hội thoại.' });
  }
});
...
router.put('/dialogues/:id', async (req, res) => {
  try {
    const { title, title_cn, level, sort_order } = req.body;
    await pool.query('UPDATE dialogues SET title=?, title_cn=?, level=?, sort_order=? WHERE id=?',
      [title, title_cn, level, sort_order || 0, req.params.id]);
    res.json({ message: 'Cập nhật hội thoại thành công.' });
  } catch (err) {
...
router.delete('/dialogues/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM dialogues WHERE id = ?', [req.params.id]);
...
// Dialogue lines
router.post('/dialogues/:id/lines', async (req, res) => {
  try {
    const { line_order, speaker, speaker_name, hanzi, pinyin, meaning } = req.body;
    if (!speaker || !speaker_name || !hanzi || !pinyin || !meaning) {
      return res.status(400).json({ error: 'Vui lòng nhập đầy đủ thông tin câu thoại.' });
    }
    const [result] = await pool.query(
      'INSERT INTO dialogue_lines (dialogue_id, line_order, speaker, speaker_name, hanzi, pinyin, meaning) VALUES (?,?,?,?,?,?,?)',
      [req.params.id, line_order || 1, speaker, speaker_name, hanzi, pinyin, meaning]
    );
    res.status(201).json({ message: 'Thêm câu thoại thành công.', id: result.insertId });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi thêm câu thoại.' });
  }
});

router.put('/dialogue-lines/:id', async (req, res) => {
  try {
    const { line_order, speaker, speaker_name, hanzi, pinyin, meaning } = req.body;
    await pool.query(
      'UPDATE dialogue_lines SET line_order=?, speaker=?, speaker_name=?, hanzi=?, pinyin=?, meaning=? WHERE id=?',
      [line_order, speaker, speaker_name, hanzi, pinyin, meaning, req.params.id]
    );
    res.json({ message: 'Cập nhật câu thoại thành công.' });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi cập nhật câu thoại.' });
  }
});

router.delete('/dialogue-lines/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM dialogue_lines WHERE id = ?', [req.params.id]);
    res.json({ message: 'Xóa câu thoại thành công.' });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi xóa câu thoại.' });
  }
});
...
router.get('/blog', async (req, res) => {
  try {
    const { sortBy = 'published_at', sortDir = 'DESC' } = req.query;
    const allowedSort = ['id', 'title', 'category', 'published_at', 'sort_order'];
    const safeSortBy = allowedSort.includes(sortBy) ? sortBy : 'published_at';
    const safeSortDir = sortDir.toUpperCase() === 'DESC' ? 'DESC' : 'ASC';
    const [rows] = await pool.query(`SELECT * FROM blog_posts ORDER BY ${safeSortBy} ${safeSortDir}, id DESC`);
    res.json({ posts: rows });
  } catch (err) {
    res.status(500).json({ error: 'Lỗi tải blog.' });
  }
});

router.post('/blog', async (req, res) => {
  try {
    const { title, excerpt, content, category, emoji, published_at } = req.body;
    if (!title) return res.status(400).json({ error: 'Vui lòng nhập tiêu đề bài viết.' });
    const [result] = await pool.query(
      'INSERT INTO blog_posts (title, excerpt, content, category, emoji, published_at) VALUES (?,?,?,?,?,?)',
      [title, excerpt || '', content || '', category || '', emoji || '📝', published_at || new Date().toISOString().split('T')[0]]
    );
...
router.put('/blog/:id', async (req, res) => {
  try {
    const { title, excerpt, content, category, emoji, published_at } = req.body;
    await pool.query(
      'UPDATE blog_posts SET title=?, excerpt=?, content=?, category=?, emoji=?, published_at=? WHERE id=?',
      [title, excerpt || '', content || '', category || '', emoji || '📝', published_at, req.params.id]
    );
...
// =============================================
router.put('/reorder/:table', async (req, res) => {
  try {
    const allowedTables = ['vocabulary', 'blog_posts', 'exam_questions', 'dialogues'];
    ...
    try {
      ...
      for (const item of items) {
        await conn.query(`UPDATE ${table} SET sort_order = ? WHERE id = ?`, [item.sort_order, item.id]);
      }
      await conn.commit();
      res.json({ message: 'Cập nhật thứ tự thành công.', count: items.length });
    } catch (e) {
      await conn.rollback();
      throw e;
    } finally {
    ...
    res.status(500).json({ error: 'Lỗi cập nhật thứ tự.' });
  }
});

// Swap two adjacent items by ID — normalizes ALL rows first to prevent conflicts
router.put('/swap/:table', async (req, res) => {
  try {
    const allowedTables = ['vocabulary', 'blog_posts', 'exam_questions', 'dialogues'];
    const table = req.params.table;
    if (!allowedTables.includes(table)) {
    ...
    const { idA, idB, sortBy = 'sort_order', sortDir = 'ASC' } = req.body;
    if (!idA || !idB) {
      return res.status(400).json({ error: 'Cần truyền idA và idB.' });
    }
    ...
    try {
      ...
      // Step 1: Normalize ALL rows — assign sequential sort_order based on current order
      const orderCol = ['id', 'sort_order', 'hanzi', 'pinyin', 'meaning', 'level', 'lesson', 'title', 'category', 'published_at'].includes(sortBy) ? sortBy : 'id';
      const dir = sortDir.toUpperCase() === 'DESC' ? 'DESC' : 'ASC';
      const [allRows] = await conn.query(`SELECT id FROM ${table} ORDER BY ${orderCol} ${dir}, id ASC`);
      for (let i = 0; i < allRows.length; i++) {
        await conn.query(`UPDATE ${table} SET sort_order = ? WHERE id = ?`, [i, allRows[i].id]);
      }

      // Step 2: Get the sort_order of both items after normalization
      const [[rowA]] = await conn.query(`SELECT sort_order FROM ${table} WHERE id = ?`, [idA]);
      const [[rowB]] = await conn.query(`SELECT sort_order FROM ${table} WHERE id = ?`, [idB]);
      ...
      // Step 3: Swap their sort_order values
      await conn.query(`UPDATE ${table} SET sort_order = ? WHERE id = ?`, [rowB.sort_order, idA]);
      await conn.query(`UPDATE ${table} SET sort_order = ? WHERE id = ?`, [rowA.sort_order, idB]);

      await conn.commit();
      res.json({ message: 'Hoán đổi thứ tự thành công.' });
    } catch (e) {
      await conn.rollback();
      throw e;
    } finally {
    ...
    res.status(500).json({ error: 'Lỗi hoán đổi thứ tự.' });
  }
});
...
async function kiemTraGiaoVien(teacher_id, req = null) {
  ...
  const [r] = await pool.query(`SELECT 1 FROM users WHERE id = ? AND role IN ('teacher','org_admin','admin')${dk}`, ts);
  ...
  try {
    ...
    if (emails.length) {
      const [[org]] = await pool.query(
        'SELECT gioi_han_hoc_vien FROM organizations WHERE id = ?', [orgLop]
      ).catch(() => [[null]]);
      // Hạn mức học viên là thứ trung tâm TRẢ TIỀN theo — trước đây chỉ hiện ra ở màn quản trị
      // chứ không chặn gì, tức gói 50 em dùng được vô hạn.
      let conTrong = Infinity;
      if (org?.gioi_han_hoc_vien) {
        const [[{ n }]] = await pool.query(
          "SELECT COUNT(*) AS n FROM users WHERE org_id = ? AND role = 'student'", [orgLop]);
        conTrong = Math.max(0, org.gioi_han_hoc_vien - n);
      }
