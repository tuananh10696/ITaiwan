    ...
    <div class="account-profile-page">
      ...
      <div class="account-card tk-lienket">
        ...
        <div class="tk-lienket-row">
          <button class="tk-lk" onclick="window.app.navigate('account-membership')">
            <i class="fa-solid fa-crown"></i><span>Gói thành viên</span></button>
...
// ============================================================
// 4. GÓI THÀNH VIÊN — bảng giá + chuyển khoản + gửi biên lai
// ============================================================
// Bán theo QUYỂN/CẤP: 17 khoá giáo trình (799.000đ giảm còn 599.000đ) + đề thi thử
// (499.000đ còn 399.000đ). Bảng giá đọc từ `products` chứ KHÔNG viết cứng trong mã — đổi giá là
// việc của DB, xem `server/config/migration-thanh-toan.sql`.
//
// Luồng: bảng giá -> bấm Mua -> tạo đơn -> màn chuyển khoản (QR + thông tin + nội dung CK) -> tải ảnh
// biên lai -> chờ quản trị duyệt -> nhận thông báo ở chuông.
//
// ⚠️ Giao diện KHÔNG được hứa "hệ thống tự kiểm tra ảnh rồi mở khoá ngay". Ảnh chỉ là bằng chứng
//    để người duyệt đối chiếu với app ngân hàng — nói khác đi là hứa sai với người trả tiền
//    (lý do đầy đủ ở đầu server/routes/thanh-toan.js).

const NHOM_TEN = {
  duongdai: 'Giáo trình Đương đại', thoidai: 'Giáo trình Thời Đại',
  hsk: 'HSK 3.0', 'thi-thu': 'Đề thi thử',
};
const NHOM_ICON = {
  duongdai: 'fa-book', thoidai: 'fa-book-open-reader', hsk: 'fa-graduation-cap', 'thi-thu': 'fa-file-pen',
};

async function renderAccountMembership(el) {
  // Bảng giá xem được khi CHƯA đăng nhập — đây là trang bán hàng, chặn ở cửa thì không ai biết
  // giá bao nhiêu. Chỉ nút Mua mới mời đăng nhập.
  el.innerHTML = `<div class="account-profile-page">
    <div class="tw-sk" style="height:120px;margin-bottom:16px"></div>
    <div class="tk-goi-grid">${'<div class="tw-sk" style="height:170px"></div>'.repeat(6)}</div>
  </div>`;

  try {
    const [bg, ch] = await Promise.all([api.getBangGia(), api.getCauHinhThanhToan()]);
    tkState.bangGia = bg.goi || [];
    tkState.cauHinh = ch;
    // Quyền và đơn hàng chỉ có nghĩa khi đã đăng nhập; gọi khi chưa đăng nhập là chắc chắn 401.
    if (state.isLoggedIn) {
      const [q, d] = await Promise.all([
        api.getQuyenCuaToi().catch(() => null),
        api.getDonCuaToi().catch(() => null),
      ]);
      tkState.quyen = q;
      tkState.don = d?.don || [];
    } else {
    ...
    el.innerHTML = `<div class="coming-soon">
      <div class="coming-soon-icon"><i class="fa-solid fa-triangle-exclamation"></i></div>
      <h2 class="coming-soon-title">Không tải được bảng giá</h2>
      <p class="coming-soon-desc">${tdEsc(err.message || 'Lỗi kết nối.')}</p>
      <button class="btn-primary" style="margin-top:8px" onclick="window.app.navigate('account-membership')">Thử lại</button>
    </div>`;
    return;
  }
  ...
  // Vào THẲNG màn thanh toán bằng URL (deep link, F5, hoặc Back/Forward) thì không ai chạy
  // `tkMoThanhToan()` — `PAGE_PARAMS.read()` chỉ đặt `tkState.muaMa`, và nó chạy TRƯỚC khi DOM
  // của trang được dựng nên KHÔNG được gọi API ở đó (bẫy 4.21b). Thiếu bước này thì màn thanh
  // toán đứng ở khung xương vĩnh viễn — mà URL vẫn đúng nên nhìn qua tưởng đang tải.
  if (tkState.muaMa && !tkState.donHienTai && state.isLoggedIn) {
    try {
      tkState.donHienTai = (await api.taoDonMua(tkState.muaMa)).don;
      tkState.loiDon = null;
    } catch (err) {
      tkState.loiDon = err.message || 'Không tạo được đơn.';
    }
    // Người dùng có thể đã sang trang khác trong lúc chờ mạng — kiểm lại trước khi vẽ.
    if (state.currentPage === 'account-membership') tkVeGoi();
  }
}

/** Đã sở hữu khoá này chưa — suy từ `pham_vi` của sản phẩm và quyền hiện có. */
function tkDaCo(sp) {
  const q = tkState.quyen;
  if (!q || !q.dang_nhap) return false;
  if (q.tat_ca) return true;
  let pv = sp.pham_vi;
  if (typeof pv === 'string') { try { pv = JSON.parse(pv); } catch { pv = {}; } }
  const bo = (pv?.bo || [])[0];
  if (!bo) return false;
  if ((q.bo || []).includes(bo)) return true;
  const quyen = (pv?.quyen || [])[0];
  return quyen != null && (q.quyen || []).includes(`${bo}:${quyen}`);
}
...
function tkVeGoi(el) {
  ...
  // Đang ở màn thanh toán của một khoá cụ thể (URL /thanh-toan/<mã>).
  if (tkState.muaMa) return tkVeThanhToan(root);

  const ds = tkState.bangGia || [];
  const loc = tkState.loc || 'all';
  const daCo = ds.filter(tkDaCo).length;

  if (loc === 'don') return tkVeDonCuaToi(root);

  // Khu HSK đang ẩn (shared/an-hsk.js) nên 6 khoá HSK + combo trọn bộ HSK không lên bảng giá.
  // Sản phẩm vẫn còn trong `products` và quyền đã bán vẫn nguyên — chỉ thôi chào bán.
  const nhoms = ['duongdai', 'thoidai', 'thi-thu'].concat(AN_HSK ? [] : ['hsk']).sort(
    (x, y) => ['duongdai', 'thoidai', 'hsk', 'thi-thu'].indexOf(x) - ['duongdai', 'thoidai', 'hsk', 'thi-thu'].indexOf(y));
  const hienNhom = loc === 'all' ? nhoms : [loc];

  const soDon = (tkState.don || []).filter((d) => d.trang_thai === 'cho').length;
  const tabs = [
    ['all', 'Tất cả'], ['duongdai', 'Đương đại'], ['thoidai', 'Thời Đại'],
    ['hsk', 'HSK'], ['thi-thu', 'Thi thử'],
  ].filter(([k]) => !(AN_HSK && k === 'hsk')).map(([k, v]) => `<button class="tk-goi-tab ${loc === k ? 'active' : ''}" onclick="window.app.tkGoiLoc('${k}')">${v}</button>`).join('')
    + (state.isLoggedIn ? `<button class="tk-goi-tab ${loc === 'don' ? 'active' : ''}" onclick="window.app.tkGoiLoc('don')">
        Đơn của tôi${soDon ? ` <span class="tk-tb-dem">${soDon}</span>` : ''}</button>` : '');

  root.innerHTML = `
    <div class="account-profile-page tk-goi-page">
      <div class="tk-goi-hero">
        <div class="tk-goi-hero-in">
          <span class="tk-goi-badge"><i class="fa-solid fa-crown"></i> Gói thành viên</span>
          <h2>Mở khoá trọn quyển, học mãi mãi</h2>
          <p>Mua một lần, sở hữu vĩnh viễn — không thuê bao, không gia hạn.
             Ba bài đầu của mọi quyển luôn miễn phí để bạn học thử trước khi quyết định.</p>
          ${state.isLoggedIn && daCo ? `<div class="tk-goi-daco"><i class="fa-solid fa-circle-check"></i>
            Bạn đang sở hữu <b>${daCo}</b> khoá</div>` : ''}
        </div>
      </div>

      <div class="tk-goi-tabs">${tabs}</div>

      ${hienNhom.map((n) => {
        const sp = ds.filter((x) => x.nhom === n);
        if (!sp.length) return '';
        return `<section class="tk-goi-nhom">
          <h3 class="tk-goi-nhom-ten"><i class="fa-solid ${NHOM_ICON[n]}"></i> ${NHOM_TEN[n] || n}</h3>
          <div class="tk-goi-grid">${sp.map(tkTheGoiHtml).join('')}</div>
        </section>`;
      }).join('')}

      <div class="tk-goi-note">
        <i class="fa-solid fa-circle-info"></i>
        <span>Thanh toán bằng chuyển khoản ngân hàng. Sau khi chuyển, bạn tải ảnh biên lai lên;
          quản trị viên đối chiếu rồi mở khoá — thường trong vài giờ.
          Cần hỗ trợ? <a href="${tdEsc(tkState.cauHinh?.lien_he || '#')}" target="_blank" rel="noopener">Liên hệ quản trị viên</a>.</span>
      </div>
    </div>`;
  twPlayEnter(root, 'tw-entering');
}

function tkTheGoiHtml(sp) {
  const co = tkDaCo(sp);
  const cho = tkDonCho(sp.ma);
  const giam = sp.gia_goc && sp.gia_goc > sp.gia
    ? Math.round((1 - sp.gia / sp.gia_goc) * 100) : 0;

  let nut;
  if (co) {
    nut = `<button class="tk-goi-btn is-owned" disabled><i class="fa-solid fa-circle-check"></i> Đã sở hữu</button>`;
  } else if (cho) {
    nut = `<button class="tk-goi-btn is-wait" onclick="window.app.tkMoThanhToan('${sp.ma}')">
      <i class="fa-solid fa-clock"></i> ${cho.co_anh ? 'Đang chờ duyệt' : 'Tiếp tục thanh toán'}</button>`;
  } else {
    nut = `<button class="tk-goi-btn" onclick="window.app.tkMoThanhToan('${sp.ma}')">
      <i class="fa-solid fa-cart-shopping"></i> Mua khoá này</button>`;
  }

  return `
    <article class="tk-goi-the ${co ? 'is-owned' : ''}">
      ${giam ? `<span class="tk-goi-sale">-${giam}%</span>` : ''}
      <h4 class="tk-goi-ten">${tdEsc(tkBoHsk(sp.ten))}</h4>
      <p class="tk-goi-mo">${tdEsc(tkBoHsk(sp.mo_ta || ''))}</p>
      <div class="tk-goi-gia">
        ${sp.gia_goc && sp.gia_goc > sp.gia ? `<span class="tk-goi-cu">${tkTien(sp.gia_goc)}</span>` : ''}
        <span class="tk-goi-moi">${tkTien(sp.gia)}</span>
      </div>
      <div class="tk-goi-vv"><i class="fa-solid fa-infinity"></i> Sở hữu vĩnh viễn</div>
      ${nut}
    </article>`;
}
...
/**
 * Ảnh mã QR chuyển khoản theo chuẩn VietQR.
 * Dịch vụ ảnh của vietqr.io mở công khai, không cần khoá API. Mã QR đã nhúng sẵn số tiền và nội
 * dung chuyển khoản nên người học không phải gõ tay — gõ tay chính là chỗ hay sai nội dung.
 * QR hỏng/không tải được thì vẫn còn bảng thông tin bên dưới, KHÔNG bao giờ chỉ dựa vào QR.
 */
function tkQrUrl(cf, don) {
  const q = new URLSearchParams({
    amount: String(don.so_tien || 0),
    addInfo: don.ma_giao_dich || '',
    accountName: cf.bank_chu_tk || '',
  });
  return `https://img.vietqr.io/image/${encodeURIComponent(cf.bank_bin)}-${encodeURIComponent(cf.bank_stk)}-compact2.png?${q}`;
}

async function tkMoThanhToan(ma) {
  if (!state.isLoggedIn) { openAuth(); return; }
  tkState.muaMa = ma;
  tkState.donHienTai = null;
  tkState.anhTam = null;
  updateUrl({ push: true });
  tkVeGoi();

  // Tạo (hoặc lấy lại) đơn SAU khi đã vẽ màn hình: người dùng thấy ngay phản hồi, không đứng
  // trước một trang trắng trong lúc chờ mạng.
  try {
    const kq = await api.taoDonMua(ma);
    tkState.donHienTai = kq.don;
  } catch (err) {
    tkState.loiDon = err.message || 'Không tạo được đơn.';
  }
  tkVeGoi();
...
function tkVeThanhToan(root) {
  const sp = (tkState.bangGia || []).find((x) => x.ma === tkState.muaMa);
  const cf = tkState.cauHinh || {};
  const don = tkState.donHienTai;

  if (!sp) {
    root.innerHTML = `<div class="coming-soon">
      <div class="coming-soon-icon"><i class="fa-solid fa-circle-question"></i></div>
      <h2 class="coming-soon-title">Không tìm thấy khoá học</h2>
      <p class="coming-soon-desc">Khoá học này có thể đã ngừng bán.</p>
      <button class="btn-primary" style="margin-top:8px" onclick="window.app.tkDongThanhToan()">Về bảng giá</button>
    </div>`;
    return;
  }

  const dauTrang = `
    <button class="tk-tt-back" onclick="window.app.tkDongThanhToan()">
      <i class="fa-solid fa-arrow-left"></i> Bảng giá
    </button>
    <div class="tk-tt-head">
      <h2>Thanh toán khoá học</h2>
      <p class="tk-tt-sp">${tdEsc(sp.ten)} · <b>${tkTien(sp.gia)}</b></p>
    </div>`;

  // Chủ hệ thống chưa điền số tài khoản -> nói thẳng, KHÔNG hiện một số tài khoản bịa.
  if (!cf.da_cau_hinh) {
    root.innerHTML = `<div class="account-profile-page tk-tt-page">${dauTrang}
      <div class="tk-tt-canh">
        <i class="fa-solid fa-triangle-exclamation"></i>
        <span><b>Chưa thể thanh toán tự động.</b> Hệ thống chưa được cấu hình tài khoản nhận tiền.
        Vui lòng <a href="${tdEsc(cf.lien_he || '#')}" target="_blank" rel="noopener">liên hệ quản trị viên</a>
        để được hướng dẫn mua khoá học này.</span>
      </div></div>`;
    twPlayEnter(root, 'tw-entering');
    return;
  }
  ...
  if (tkState.loiDon) {
    root.innerHTML = `<div class="account-profile-page tk-tt-page">${dauTrang}
      <div class="tk-tt-canh">
        ...
        <span>${tdEsc(tkState.loiDon)}</span>
      </div>
      <button class="btn btn-accent" onclick="window.app.tkMoThanhToan('${sp.ma}')">Thử lại</button>
    </div>`;
    twPlayEnter(root, 'tw-entering');
    return;
  }
  ...
  if (!don) {
    root.innerHTML = `<div class="account-profile-page tk-tt-page">${dauTrang}
      <div class="tw-sk" style="height:320px"></div></div>`;
    return;
  }

  const daGui = !!don.co_anh;
  const anh = tkState.anhTam;

  root.innerHTML = `
    <div class="account-profile-page tk-tt-page">
      ${dauTrang}

      <div class="tk-tt-buoc">
        <div class="tk-tt-b ${daGui ? 'xong' : 'dang'}"><span>1</span> Chuyển khoản</div>
        <div class="tk-tt-b ${daGui ? 'xong' : ''}"><span>2</span> Gửi biên lai</div>
        <div class="tk-tt-b"><span>3</span> Chờ duyệt</div>
      </div>

      <div class="tk-tt-grid">
        <div class="tk-tt-qr">
          <img src="${tdEsc(tkQrUrl(cf, don))}" alt="Mã QR chuyển khoản"
               onerror="this.closest('.tk-tt-qr').classList.add('loi')">
          <p class="tk-tt-qr-mo">Quét bằng app ngân hàng — số tiền và nội dung đã điền sẵn.</p>
          <!-- Dự phòng: vietqr.io là dịch vụ NGOÀI, nó chết thì cả trang thanh toán mất mã quét.
               Ảnh tĩnh này trỏ cùng tài khoản nhưng KHÔNG nhúng được số tiền / nội dung, nên đi
               kèm lời nhắc tự nhập — đừng bỏ lời nhắc đó đi. -->
          <div class="tk-tt-qr-loi">
            <img src="/images/qr-chuyen-khoan.jpg" alt="Mã QR ngân hàng">
            <p>Mã QR tự động tạm lỗi — dùng mã này nhé. Nhớ <b>tự nhập số tiền và nội dung</b>
               theo bảng bên cạnh.</p>
          </div>
        </div>

        <div class="tk-tt-tt">
          ${tkDongTinHtml('Ngân hàng', cf.bank_ten || '—', null)}
          ${tkDongTinHtml('Số tài khoản', cf.bank_stk, cf.bank_stk)}
          ${tkDongTinHtml('Chủ tài khoản', cf.bank_chu_tk || '—', null)}
          ${tkDongTinHtml('Số tiền', tkTien(don.so_tien), String(don.so_tien))}
          ${tkDongTinHtml('Nội dung chuyển khoản', don.ma_giao_dich, don.ma_giao_dich, true)}
          <div class="tk-tt-luuy">
            <i class="fa-solid fa-circle-exclamation"></i>
            <span>Quét mã QR thì nội dung đã điền sẵn. Nếu chuyển khoản thủ công, hãy ghi đúng
              <b>${tdEsc(don.ma_giao_dich)}</b> — đây là chỗ để nhận ra đơn của bạn.
              App ngân hàng có tự bỏ dấu <b>@</b> hay <b>.</b> cũng không sao.</span>
          </div>
        </div>
      </div>

      <div class="account-card tk-tt-bill">
        <h3><i class="fa-solid fa-receipt"></i> Gửi ảnh biên lai</h3>
        ${daGui ? `
          <div class="tk-tt-daxong">
            <i class="fa-solid fa-circle-check"></i>
            <div>
              <b>Đã nhận biên lai của bạn.</b>
              <p>Quản trị viên sẽ đối chiếu với sao kê ngân hàng rồi mở khoá — thường trong vài giờ.
                 Bạn sẽ nhận được thông báo ở chuông khi xong.</p>
            </div>
          </div>
          <button class="btn tk-btn-phu" onclick="window.app.tkChonAnh()">
            <i class="fa-solid fa-rotate"></i> Gửi lại ảnh khác
          </button>` : `
          <p class="tk-cai-mo">Sau khi chuyển khoản, chụp màn hình biên lai rồi tải lên đây.
            Ảnh cần thấy rõ <b>số tiền</b> và <b>nội dung chuyển khoản</b>.</p>`}

        <input type="file" id="tk-anh-input" accept="image/*" hidden onchange="window.app.tkAnhDoi(event)">

        ${anh ? `
          <div class="tk-tt-xem">
            <img src="${anh}" alt="Ảnh biên lai đã chọn">
            <div class="tk-tt-xem-nut">
              <button class="btn btn-accent" ${tkState.dangGui ? 'disabled' : ''} onclick="window.app.tkGuiAnh()">
                <i class="fa-solid ${tkState.dangGui ? 'fa-spinner fa-spin' : 'fa-paper-plane'}"></i>
                ${tkState.dangGui ? 'Đang gửi…' : 'Gửi biên lai'}
              </button>
              <button class="btn tk-btn-phu" onclick="window.app.tkBoAnh()">Chọn ảnh khác</button>
            </div>
          </div>` : (daGui ? '' : `
          <button class="tk-tt-tai" onclick="window.app.tkChonAnh()">
            <i class="fa-solid fa-cloud-arrow-up"></i>
            <span>Chọn ảnh biên lai</span>
            <small>JPG hoặc PNG</small>
          </button>`)}

        <div class="auth-error" id="tk-anh-err"></div>
      </div>

      <div class="tk-goi-note">
        <i class="fa-solid fa-headset"></i>
        <span>Đã chuyển khoản nhưng chưa được mở khoá sau 24 giờ, hoặc gặp bất kỳ vấn đề nào?
          <a href="${tdEsc(cf.lien_he || '#')}" target="_blank" rel="noopener">Nhắn cho quản trị viên</a>
          kèm nội dung <b>${tdEsc(don.ma_giao_dich)}</b>.</span>
      </div>
    </div>`;
  twPlayEnter(root, 'tw-entering');
}

function tkDongTinHtml(nhan, giaTri, chep, nhanManh = false) {
  return `<div class="tk-tt-dong ${nhanManh ? 'manh' : ''}">
    <span class="tk-tt-nhan">${tdEsc(nhan)}</span>
    <span class="tk-tt-gt">${tdEsc(giaTri)}</span>
    ${chep ? `<button class="tk-tt-chep" title="Sao chép" onclick="window.app.tkChep(this, '${tdEsc(chep)}')">
      <i class="fa-solid fa-copy"></i></button>` : ''}
  </div>`;
}

/** Sao chép có DỰ PHÒNG: `navigator.clipboard` cần ngữ cảnh bảo mật, trong WebView app hoặc
 *  trang http nó ném lỗi im lặng và người dùng tưởng nút hỏng. */
async function tkChep(btn, giaTri) {
  let xong = false;
  try {
  ...
  if (xong) {
    ...
    setTimeout(() => { btn.innerHTML = '<i class="fa-solid fa-copy"></i>'; }, 1400);
    toast('Đã sao chép.');
  } else {
    toast('Không sao chép được — bạn chép tay giúp nhé.');
  }
}

const tkChonAnh = () => document.getElementById('tk-anh-input')?.click();

async function tkAnhDoi(ev) {
  const f = ev.target?.files?.[0];
  if (!f) return;
  const loi = document.getElementById('tk-anh-err');
  if (loi) loi.style.display = 'none';
  if (!f.type.startsWith('image/')) {
    if (loi) { loi.textContent = 'Vui lòng chọn một tệp ảnh.'; loi.style.display = 'block'; }
    return;
  }
  try {
    tkState.anhTam = await tkNenAnh(f);
    tkVeGoi();
  } catch (err) {
    if (loi) { loi.textContent = err.message || 'Không đọc được ảnh.'; loi.style.display = 'block'; }
  }
}

function tkBoAnh() { tkState.anhTam = null; tkVeGoi(); }

async function tkGuiAnh() {
  const don = tkState.donHienTai;
  if (!don || !tkState.anhTam || tkState.dangGui) return;
  tkState.dangGui = true;
  tkVeGoi();
  try {
    await api.guiBienLai(don.id, tkState.anhTam);
    tkState.dangGui = false;
    tkState.anhTam = null;
    tkState.donHienTai = { ...don, co_anh: true };
    // Danh sách đơn đang giữ bản cũ — cập nhật để tab "Đơn của tôi" khớp ngay, khỏi gọi lại API.
    const i = (tkState.don || []).findIndex((d) => d.id === don.id);
    if (i >= 0) tkState.don[i] = { ...tkState.don[i], co_anh: true, anh_gui_luc: new Date().toISOString() };
    tkVeGoi();
    toast('Đã gửi biên lai. Chờ quản trị viên duyệt nhé!');
  } catch (err) {
    tkState.dangGui = false;
    tkVeGoi();
    // KHÔNG nuốt lỗi: bài dịch từng im lặng không lưu suốt nhiều tháng vì đúng lỗi này (4.21b).
    const loi = document.getElementById('tk-anh-err');
    if (loi) { loi.textContent = err.message || 'Không gửi được ảnh. Thử lại giúp nhé.'; loi.style.display = 'block'; }
  }
}
...
/**
 * Bỏ phần nhắc HSK khỏi tên/mô tả sản phẩm khi khu HSK đang ẩn.
 * Gói "Đề thi thử" gồm cả đề HSK lẫn TOCFL, nhưng người mua nay chỉ vào được phần TOCFL — để
 * nguyên chữ "TOCFL + HSK" trên bảng giá là hứa thứ họ không dùng được.
 * ⚠️ Chỉ đổi CHỮ HIỂN THỊ. Bản ghi trong `products` giữ nguyên, quyền đã bán giữ nguyên.
 */
function tkBoHsk(s) {
  if (!AN_HSK) return s;
  return String(s || '')
    .replace(/\s*(?:\+|và|·|,)\s*HSK\b/gi, '')
    .replace(/\bHSK\s*(?:\+|và|·)\s*/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

function tkVeDonCuaToi(root) {
  const ds = tkState.don || [];
  const tabs = [
    ['all', 'Tất cả'], ['duongdai', 'Đương đại'], ['thoidai', 'Thời Đại'],
    ['hsk', 'HSK'], ['thi-thu', 'Thi thử'], ['don', 'Đơn của tôi'],
  ].filter(([k]) => !(AN_HSK && k === 'hsk')).map(([k, v]) => `<button class="tk-goi-tab ${k === 'don' ? 'active' : ''}" onclick="window.app.tkGoiLoc('${k}')">${v}</button>`).join('');

  root.innerHTML = `
    <div class="account-profile-page tk-goi-page">
      <div class="tk-goi-tabs">${tabs}</div>
      ${ds.length ? `<div class="tk-don-list">${ds.map(tkDonHtml).join('')}</div>` : `
        <div class="coming-soon">
          <div class="coming-soon-icon"><i class="fa-solid fa-receipt"></i></div>
          <h2 class="coming-soon-title">Chưa có đơn nào</h2>
          <p class="coming-soon-desc">Các khoá học bạn đã mua hoặc đang chờ duyệt sẽ hiện ở đây.</p>
          <button class="btn-primary" style="margin-top:8px" onclick="window.app.tkGoiLoc('all')">Xem bảng giá</button>
        </div>`}
    </div>`;
  twPlayEnter(root, 'tw-entering');
}

function tkDonHtml(d) {
  const tt = TT_NHAN[d.trang_thai] || { t: d.trang_thai, c: '', i: 'fa-circle' };
  const cho = d.trang_thai === 'cho';
  return `
    <article class="tk-don ${tt.c}">
      <div class="tk-don-dau">
        <h4>${tdEsc(d.san_pham_ten || d.product_ma)}</h4>
        <span class="tk-don-tt ${tt.c}"><i class="fa-solid ${tt.i}"></i> ${tt.t}</span>
      </div>
      <div class="tk-don-meta">
        <span>Nội dung CK: <b>${tdEsc(d.ma_giao_dich || '—')}</b></span>
        <span>${tkTien(d.so_tien)}</span>
        <span>${tkNgay(d.created_at)}</span>
      </div>
      ${cho && !d.co_anh ? `<p class="tk-don-nhac">
        <i class="fa-solid fa-circle-exclamation"></i> Bạn chưa gửi ảnh biên lai cho đơn này.</p>` : ''}
      ${cho && d.co_anh ? `<p class="tk-don-nhac ok">
        <i class="fa-solid fa-hourglass-half"></i> Đã gửi biên lai, đang chờ quản trị viên đối chiếu.</p>` : ''}
      ${d.trang_thai === 'tu-choi' && d.ghi_chu ? `<p class="tk-don-nhac no">
        <i class="fa-solid fa-circle-xmark"></i> Lý do: ${tdEsc(d.ghi_chu)}</p>` : ''}
      <div class="tk-don-nut">
        ${cho ? `<button class="btn tk-btn-phu" onclick="window.app.tkMoThanhToan('${d.product_ma}')">
          <i class="fa-solid fa-qrcode"></i> ${d.co_anh ? 'Xem lại' : 'Tiếp tục thanh toán'}</button>` : ''}
        ${cho ? `<button class="btn tk-btn-huy" onclick="window.app.tkHuyDon(${d.id})">Huỷ đơn</button>` : ''}
        ${d.trang_thai === 'tu-choi' ? `<button class="btn btn-accent" onclick="window.app.tkMoThanhToan('${d.product_ma}')">
          <i class="fa-solid fa-rotate"></i> Mua lại</button>` : ''}
      </div>
    </article>`;
}

async function tkHuyDon(id) {
  openDialog('Huỷ đơn?',
    '<p>Bạn có chắc muốn huỷ đơn này? Nếu đã chuyển khoản rồi thì <b>đừng huỷ</b> — hãy gửi ảnh biên lai và chờ duyệt.</p>',
    `<button class="btn tk-btn-phu" onclick="window.app.closeDialog()">Không huỷ</button>
     <button class="btn tk-btn-huy" onclick="window.app.tkHuyDonThat(${id})">Huỷ đơn</button>`);
}

async function tkHuyDonThat(id) {
  closeDialog();
  try {
    await api.huyDonMua(id);
    const d = (tkState.don || []).find((x) => x.id === id);
    if (d) d.trang_thai = 'that-bai';
    if (tkState.donHienTai?.id === id) { tkState.muaMa = null; tkState.donHienTai = null; }
    tkVeGoi();
    toast('Đã huỷ đơn.');
  } catch (err) {
    toast(err.message || 'Không huỷ được đơn.');
  }
}
...
export const render = {
  ...
  'account-membership': renderAccountMembership,
...
export const handlers = {
  ...
  tkGoiLoc, tkMoThanhToan, tkDongThanhToan, tkChep,
  tkChonAnh, tkAnhDoi, tkBoAnh, tkGuiAnh,
  tkHuyDon, tkHuyDonThat,
